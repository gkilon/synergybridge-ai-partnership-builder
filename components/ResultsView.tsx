    ...
    <div className="space-y-12 animate-fadeIn pb-32 max-w-[1700px] mx-auto px-4 text-right" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-zinc-900 pb-10">
        <div className="space-y-2">
           ...
           <p className="text-zinc-500 font-bold text-xs uppercase tracking-[0.4em]">אבחון ממשק עבודה אסטרטגי</p>
        </div>
        <div className="flex gap-4">
           <button onClick={onBack} className="bg-zinc-900 text-zinc-500 px-8 py-4 rounded-2xl font-black border border-zinc-800 hover:text-white transition-all">חזרה</button>
           <button onClick={handleAnalyze} disabled={loading} className={`px-10 py-4 rounded-2xl font-black transition-all ${loading ? 'bg-zinc-800 text-zinc-500' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-2xl shadow-indigo-600/30 active:scale-95'}`}>
             {loading ? 'מנתח נתונים...' : '✨ ניתוח ניהולי (AI)'}
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-10">
          <div className="bg-[#09090b] rounded-[3rem] p-12 border border-white/5 shadow-3xl flex flex-col md:flex-row justify-between items-center gap-10 overflow-hidden relative">
             ...
             <div className="space-y-4 relative z-10 text-right">
                <h3 className="text-3xl font-black text-white">מדד בריאות הממשק</h3>
                <p className="text-zinc-500 text-base max-w-sm leading-relaxed">
                   ציון משוקלל המבטא את האפקטיביות ושביעות הרצון כפי שדווחו על ידי שני הצדדים.
                   {analysisSummary.biggestGap && (
                     <span className="block mt-4 text-rose-400 font-bold bg-rose-500/10 p-3 rounded-xl border border-rose-500/20">
                       🚨 זוהה פער תפיסה משמעותי בדרייבר: {analysisSummary.biggestGap.label}
                     </span>
                   )}
                </p>
             </div>
             <div className="text-center relative z-10">
                <span className="text-[12rem] font-black text-white leading-none tracking-tighter drop-shadow-2xl">
                  {analysisSummary.satisfactionScore}%
                </span>
             </div>
          </div>

          <div className="bg-[#09090b] rounded-[3.5rem] p-12 border border-white/5 shadow-3xl h-[600px]">
             <h3 className="text-2xl font-black text-white mb-8 border-r-4 border-indigo-500 pr-5">ניתוח דרייברים</h3>
             <ResponsiveContainer width="100%" height="85%">
                <RadarChart data={analysisSummary.driverData}>
                  <PolarGrid stroke="#1a1a1e" />
                  <PolarAngleAxis dataKey="subject" tick={{ fill: '#71717a', fontSize: 14, fontWeight: 900 }} />
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#000', border: 'none', borderRadius: '15px', color: '#fff' }} />
                </RadarChart>
             </ResponsiveContainer>
          </div>
        </div>

        {/* LEFT: STRATEGIC ANALYSIS (INSIGHTS) */}
        <div className="lg:col-span-5 space-y-8">
          {!session.analysis ? (
            <div className="bg-[#09090b] rounded-[3.5rem] p-16 border-dashed border-2 border-zinc-800 text-center flex flex-col items-center justify-center min-h-[500px] opacity-40">
               <div className="text-6xl mb-6">📊</div>
               <h3 className="text-xl font-black text-white">ממתין לניתוח שורה תחתונה</h3>
               <p className="text-zinc-500 max-w-xs mx-auto mt-2">לחץ על הכפתור כדי לתרגם את הגרפים לתובנות ניהוליות וצעדי עבודה.</p>
            </div>
          ) : (
            <div className="space-y-8 animate-slideDown">
              ...
              <div className="bg-indigo-600 rounded-[2.5rem] p-10 text-white shadow-4xl relative overflow-hidden group">
                 ...
                 <h3 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70 mb-4">אבחון הממשק (Summary)</h3>
                 <p className="text-2xl font-black leading-tight relative z-10">{session.analysis.summary}</p>
              </div>

              {/* RECOMMENDATIONS WITH DRILL-DOWN */}
              <div className="bg-[#0c0c0e] rounded-[3rem] p-10 border border-white/5 space-y-8">
                 <h3 className="text-2xl font-black text-white flex items-center gap-4">
                    <span className="w-10 h-10 bg-indigo-500 rounded-xl flex items-center justify-center text-xl">🚀</span>
                    צעדים לביצוע בשטח
                 </h3>
                 <div className="space-y-6">
                    {allRecs.map((rec, i) => (
                      <div key={i} className="space-y-4">
                         <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-indigo-500/30 transition-all">
                            <p className="text-lg font-bold text-zinc-100 flex-grow">{rec}</p>
                            <button 
                              onClick={() => handleExpandRec(rec)}
                              disabled={expandingRec === rec}
                              className={`shrink-0 px-6 py-3 rounded-2xl text-xs font-black transition-all ${expandedSteps[rec] ? 'bg-zinc-800 text-zinc-500 cursor-default' : 'bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500 hover:text-white border border-indigo-500/20'}`}
                            >
                              {expandingRec === rec ? 'מפרט...' : expandedSteps[rec] ? 'צעדים מוכנים 👇' : 'איך עושים את זה תכל\'ס?'}
                            </button>
                         </div>
                         
                         {/* DRILL-DOWN STEPS */}
                         {expandedSteps[rec] && (
                           <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-3xl p-8 space-y-4 animate-slideDown">
                              <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2">תכנית פעולה מפורטת:</h4>
                              {expandedSteps[rec].map((step, idx) => (
                                <div key={idx} className="flex gap-4 items-start">
                                   <span className="w-6 h-6 bg-indigo-500/20 text-indigo-400 rounded-lg flex items-center justify-center text-[10px] font-black shrink-0 mt-1">{idx+1}</span>
                                   <p className="text-sm font-bold text-zinc-300 leading-relaxed">{step}</p>
                                </div>
                              ))}
                           </div>
                         )}
